...
```bash
git almanac year /path/to/repository
```
...
```bash
git almanac year --format svg --output activity.svg
```

HTML is a self-contained responsive page with an embedded accessible SVG:

```bash
git almanac year --format html --theme dark --output activity.html
open activity.html
```
...
```bash
git almanac year --format json --output activity.json
```
...
If Git Almanac reports an invalid ref or malformed date, correct the named value and rerun it. It does not fall back to another ref or silently reinterpret a date.
